...
import math
from math import sqrt,cos,sin,radians,log
...
class CalculateBirdcage:
    ...
    def _calcGeometry(self):
        ...
        if self.coil_shape == self.parent.ELLIPSE:
            ...
            # n2 = 0
            # n3 = 1
            # while n3 <= self._division:
            #     n2 = (n3 - 1) * self.delta
            #     n += self.delta * math.sqrt(1.0 + math.pow(b / a * n2, 2.0) / (a * a - math.pow(n2, 2.0)))
            #     n3 += 1
            # self.er_segment_length = n * 4 / self.nr_of_legs
            #
            # # calculate equidistant points on the perimeter
            # i = 1
            # while i <= self.nr_of_legs / 4:
            #     n4 = 0
            #     n5 = self.er_segment_length / 2 * (2 * i - 1)
            #     n6 = 1
            #     while n5 - n4 > 0.0:
            #         n4 += self.delta * math.sqrt(1.0 + math.pow(b / a * n2, 2.0) / (a * a - math.pow(n2, 2.0)))
            #         n2 = (n6 - 1) * self.delta
            #         n6 += 1
            #     self.xcoords[int(self.nr_of_legs / 4 - i)] = n2
            #     self.ycoords[int(self.nr_of_legs / 4 - i)] = abs(math.sqrt(b * b * (1.0 - n2 / a * (n2 / a))))
            #     self.thetas[int(self.nr_of_legs / 4 - i)] = math.atan(self.ycoords[int(self.nr_of_legs / 4 - i)] / self.xcoords[int(self.nr_of_legs / 4 - i)])
            #     self.radius[int(self.nr_of_legs / 4 - i)] = math.sqrt(math.pow(self.xcoords[int(self.nr_of_legs / 4 - i)], 2.0) + math.pow(self.ycoords[int(self.nr_of_legs / 4 - i)], 2.0))
            #     i += 1

            # calculate perimeter of the ellipse
            h = (a - b) ** 2 / (a + b) ** 2
            p = math.pi * (a + b) * (1 + ((3 * h) / (10 + math.sqrt(4 - 3 * h))))
            self.er_segment_length = p / self.nr_of_legs
            ...
            for i in range(1, int(self.nr_of_legs / 4)+1):
                dist = 0
                while (dist < (self.er_segment_length/2 if i==1 else self.er_segment_length)):
                    angle += 0.01
                    ...
                    x0 = x
                    y0 = y
                self.thetas[int(self.nr_of_legs / 4 - i)] = radians(angle)
                self.xcoords[int(self.nr_of_legs / 4 - i)] = x
                self.ycoords[int(self.nr_of_legs / 4 - i)] = y
                self.radius[int(self.nr_of_legs / 4 - i)] = dist

            self.thetas = self.thetas[3::-1] + self.thetas[4:]
            self.xcoords = self.xcoords[3::-1] + self.xcoords[4:]
            self.ycoords = self.ycoords[3::-1] + self.ycoords[4:]
            self.radius = self.radius[3::-1] + self.radius[4:]
    ...
    def _exportResults(self):
        ...
        if self.coil_shape == self.parent.ELLIPSE: # and not self.shortaxis:  # todo add export multiple C's @ ellipse
            for i in range(int(self.nr_of_legs / 4)):
                self.parent.guiTabResults.v_cap_res[i].set(self.cap[i])
        else:
            self.parent.guiTabResults.v_cap_res[0].set(self.cap[int(self.nr_of_legs / 4 - 1)])

        result = f"\nResults:"
        if self.coil_shape == self.parent.ELLIPSE:
            for i in range(int(self.nr_of_legs / 4)):
                result += f"\n\tResult Capacitor #{i+1}: {self.parent.guiTabResults.v_cap_res[i].get():.2f} pF"
        else:
            result += f"\n\tResult Capacitor: {self.parent.guiTabResults.v_cap_res[0].get()} pF"

        result += f"\n\tResult ER Segment Length: {self.parent.guiTabSettings.v_er_seg_length.get()} cm"
        ...
        result += f"\n\tResult Effective Inductance Legs: {self.parent.guiTabMoreInfo.v_ind_eff_legs.get()} nH"
